...
import {requestAllBmi} from '@/services/bmiService';
...
const BmiRecordsScreen: React.FC = ({navigation}: any) => {
            ...
            renderItem={({item}) => {
              return (
                <GradientView
                  ...
                  <TouchableOpacity
                    onPress={() =>
                      navigation.navigate(NAVIGATION_ROUTES.MAIN.BmiListing, {
                        bmi_id: item._id,
                      })
                    }>
                    <Text style={styles.dataText}>Age: {item.age}</Text>
                    <Text style={styles.dataText}>Weight: {item.weight}</Text>
